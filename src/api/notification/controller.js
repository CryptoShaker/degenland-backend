...
exports.getInviteState = async (req_, res_) => {
  ...
  const resData = await Notification.find({ accountId: otherAccountId, 'playerInfo.accountId': myAccountId, state: 'unread' });
...
exports.setRead = async (req_, res_) => {
  ...
  const g_accountId = req_.body.accountId;

  const g_notificationList = await Notification.find({ accountId: g_accountId, state: 'unread' });
  console.log("---------setRead - 2 : -------", g_notificationList);
